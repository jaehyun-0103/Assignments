...
1. 프로그래밍
    ...
    1.3. [5장 함수 사용과 변수의 수명](#함수-사용과-변수의-수명)
    
    1.4. [6장 배열](#배열)
    
    1.5. [7장 포인터](#포인터)
    
    1.6. [8장 포인터와 배열의 관계](#포인터와-배열의-관계)
    
    1.7. [9장 문자열 다루기](#문자열-다루기)
    
    1.8. [10장 구조체](#구조체)
    
    1.9. [11장 전처리기와 분할 컴파일](#전처리기와-분할-컴파일)
...
## 5-1번
사용자에게 세 개의 정수를 입력 받은 후 min_max() 함수를 호출하여
최댓값과 최솟값을 출력하는 프로그램을 작성하시오.
...
## 5-2번
음수를 입력하기 전까지 0이나 양수를 입력받고, 입력을 받을 때마다 현재 입력한 상황에서
최솟값을 출력하는 프로그램을 작성하시오.

### 조건
    1. 프로그램은 루프를 돌면서 반드시 current_min() 함수를 호출해야 한다. 
...
### 조건
    1. 입력 받은 지수(b)만큼 재귀 함수를 호출하여 계산한다.
...
## 6-2번
8명의 후보(1 ~ 8번)가 출마했을 때 10명의 투표 결과를 입력 받아서
당선된 후보의 번호나 당선인의 유무를 출력하는 프로그램을 작성하시오.

### 조건
    1. 당선이 확정되려면 과반수 득표에 성공해야 한다.
...
## 7-1번
a, b를 더해 * p에 저장하는 add() 함수를 작성한 후에 main()에서 이 함수를 호출하여
...
순환 스왑을 이용한 cyclic_swap() 함수를 호출하여 main()에서 a, b, c 값을 출력하시오.
...
## 8-1번
main()에서 int타입의 num, * p, ** q를 선언하고 출력하시오.
...
reverse_array() 함수를 호출하여 입력받은 순서와 반대로 b배열에 저장하시오.
또한, 저장된 b배열의 요소는 main()에서 순서대로 출력하시오.
...
### 조건
    ...
    2. count_vowel() 함수를 호출해야 하고 함수 내부에서 포인터 산술 연산을 사용한다.
...
### 조건
    1. print_array() 함수로 데이터가 저장된 2개의 배열을 출력한다.
    2. 이후에 두 배열을 데이터를 교환하여 저장하는 swap_array() 함수를 호출하여 값을 서로 바꾸도록 하고,
       변경되었는지 확인할 수 있도록 main()에서 print_array() 함수를 호출한다.
    3. 배열의 요소 수와 입력 데이터는 5개로 한다.

---

## 8-6번
음수를 입력 받기 이전까지의 데이터를 배열에 저장하고 min_max() 함수를 호출하여
입력받은 정수 중에서 최솟값과 최댓값을 출력하시오.

### 조건
    1. 입력 값이 모두 저장되어 있는 arr 배열을 넘겨받아서 최솟값을 * min에, 최대값을 * max에 저장한다.
...
### 조건
    1. 두 개의 문자 배열은 아래와 같다.
        ▹char str1[20] = "apple";
        ▹char str2[20] = "banana";
    2. 함수는 문자열을 바꾸는 작업만 수행하며 문자열의 출력은 main() 함수에서 수행한다.
    3. swap_string() 함수를 호출하여 문자열을 교환한다.
...
### 조건
    1. main()에서 입력 받은 두 개의 문자열을 my_strcmp() 함수를 통하여 비교하고 출력한다.
    2. 코드 작성시 strcmp() 함수는 직접 사용하지 않고
       동일한 기능을 하는 my_strcmp() 함수를 구현한다.
...
### 조건
    ...
    2. main()에서 두 개의 문자열을 입력받은 후, my_strcat() 함수를 호출하여 결과를 출력한다.
    3. 코드 작성시 strcat() 함수는 직접 사용하지 않고
       동일한 기능을 하는 my_strcat() 함수를 구현한다.
...
### 조건
    1. main()에서 gets로 입력을 받고 문자열 src를 채운 뒤 my_strcpy() 함수를 호출하여
       복사하고 main()으로 되돌아와서 puts로 dest배열을 출력한다.
...
## 10-2번
3명의 이름, 나이, 성별을 키보드로 입력 받고 입력한 나이로 오름차순 정렬하시오.

### 조건
    1. 0미만 120초과되는 숫자를 입력할 경우에는 오류 메시지를 출력하여 다시 입력받도록 한다.

---

## 10-3번
3명의 이름, 나이, 성별을 키보드로 입력 받고 입력한 이름을 비교하여
아스키코드 값에 따라 오름차순으로 출력하시오.
...
## 10-4번
3명의 정보를 입력한 후 사용자에게 나이와 이름 중 원하는 기준을 선택하게 한 후 오름차순 정렬하시오.
...
### 조건
    ...
    2. 입출력 함수는 각각 myInput(), myOutput() 이다.
