...
자료구조3
➢ 프로그램 이차원 배열 (2022.04.13.)
지표면의 고도를 나타내는 데이터를 입력 받아 이미지로 표현하는 프로그램을 작성하시오. 입력 받은 데이터를 RGB 값으로 계산한 이후 새로운 이미지 파일을 생성하여 이를 출력하는 프로그램으로 플로우는 다음과 같다.
...
- 계좌 출금 기능
- 계좌 이체 기능
- 고객 관리 기능 1 : ATM 잔고, 고객 잔고 총액, 평균, 최고 값 출력
- 고객 관리 기능 2 : 계좌 목록 출력( 잔액 내림차순 정렬 ) - 알고리즘 구현
...
2. 좌석 예약 현황 보기
1. 좌석 예약 기능
2. 좌석 예약 취소 (결제 완료 시 예약 취소 불가능)
3. 티켓 결제 기능 
4. 관리 통계 기능
    
---
    
➢ 다형성 활용 문제 (2022.05.17.)
- 전투용품 샵에서 구매한 전투용 무기를 사용하는 프로그램이다. Tank, Fighter, Missile 무기는 Weapon 클래스를 상속받는다. 그리고 BattleShop은 Shop 클래스를 상속받는다. 한 페이지 내에 모든 프로그램을 완성하시오.
...
- 조건
입력 : 파일로 두 행렬을 입력, fopen 사용
출력 : 두 행렬의 곱에 대한 결과 화면 출력
...
- 원형큐 : 1차원 배열을 이용한 원형큐 구현.

- 필요함수
1. Create : 새로운 empty 큐 생성. 큐는 선언된 배열을 사용하되, front 와 rear의 인덱스 초기화
2. Empty : 큐가 empty 인지 확인. 만약 empty면 1 아니면 0 리턴
3. Full : 큐가 full 인지 확인. 만약 full 이면 1 아니면 0 리턴
4. Enqueuer : 새로운 데이터를 queue rear에 삽입 (이 연산은 큐가 full인경우 불가능하니 적절한 메시지 출력
5. Dequeuer : front에 위치한 데이터 삭제하고 리턴하여 출력 (이연산은 큐가 empty인 경우 불가능하니 적절한 메시지 출력
6. QueueDisplay : 큐에 저장된 모든 데이터 출력. 만약 empty일 경우 메시지 출력

- 위 함수는 while loop을 사용하여 사용자가 원하는 동안 enqueue와 dequeue가 동작되어야 함.
